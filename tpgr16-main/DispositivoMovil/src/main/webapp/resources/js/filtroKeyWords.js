function filtrar() {
    ...
    if (selectedKeywords.length > 0) {
        const newUrl = `/DispositivoMovil/ofertas?keywords=${selectedKeywords.join('_')}`;
        window.location.href = newUrl;
    } else {
        const newUrl = `/DispositivoMovil/ofertas`;
        window.location.href = newUrl;
    }
}
...
function limpiar() {
    ...
    if (window.location != `/DispositivoMovil/ofertas`){
		const newUrl = `/DispositivoMovil/ofertas`;
        window.location.href = newUrl;
	}
}
